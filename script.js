...
class TileMapBuilder {
  ...
  initializeDefaultTextures() {
    const defaultTextures = [
      { id: "grass", name: "Grama", color: "#4CAF50", weight: 18 },
      { id: "sand", name: "Areia", color: "#F4A460", weight: 1 },
      { id: "water", name: "Água", color: "#2196F3", weight: 20 },
      { id: "stone", name: "Pedra", color: "#AAA", weight: 2 },
      { id: "snow", name: "Neve", color: "#FFF", weight: 1 },
    ];
    ...
    this.neighborhoodRules = {
      grass: ["grass", "sand", "stone"],
      sand: ["grass", "sand", "water"],
      water: ["sand", "water"],
      stone: ["stone", "grass", "snow"],
      snow: ["snow", "stone"],
    };
  }
  ...
  randomFillMap() {
    ...
    for (let y = 0; y < this.mapHeight; y++) {
      for (let x = 0; x < this.mapWidth; x++) {
        ...
        let texturePlaced = false;
        let attempts = 0;

        while (!texturePlaced && attempts < maxAttemptsPerTile) {
          const texture = this.pickWeighted();

          if (this.isValidPlacement(x, y, texture.id)) {
            this.mapData[y][x] = texture.id;
            this.tileElements[y][x].style.backgroundColor = texture.color;
            placedTiles++;
            texturePlaced = true;
          }
          attempts++;
        ...
        if (!texturePlaced) {
          this.mapData[y][x] = "unfillable";
          this.tileElements[y][x].style.backgroundColor = "red";
          console.warn(`Could not place a valid texture at (${x}, ${y}) after ${maxAttemptsPerTile} attempts.`);
        }
      }
    }
  ...
  pickWeighted(texturesToTry = this.textures) {
    const total = texturesToTry.reduce((a, b) => a + b.weight, 0);
    let rand = Math.random() * total;
    for (const tex of texturesToTry) {
      if (rand < tex.weight) {
        return tex;
      }
      rand -= tex.weight;
  ...
  openTextureModal() {
    ...
    document.getElementById("textureColor").value = "#000000";
  ...
  saveTexture() {
    ...
    document.querySelectorAll('#neighborOptions input[type="checkbox"]:checked').forEach((checkbox) => {
      if (checkbox.value == "self") {
        selectedNeighbors.push(id);
        return;
      }
  ...
  buildOptionSelf(id = "self") {
    const isChecked = this.neighborhoodRules[id]?.includes(id) ? "checked" : "";
    const option = document.createElement("div");
    option.className = "neighbor-option";
    option.innerHTML = `
            <input type="checkbox" id="neighbor_${id}" value="${id}" ${isChecked}>
            <div class="texture-preview" style="background-color: blue; width: 20px; height: 20px;"></div>
            <label for="neighbor_${id}">Permitir auto-repetição</label>
        `;
    return option;
  }
  ...
  updateNeighborOptions(excludeId = null) {
    ...
    const selfOption = this.buildOptionSelf();
    container.appendChild(selfOption);
  }
  ...
  openRulesModal(textureId) {
    ...
    const selfOption = this.buildOptionSelf(textureId);
    container.appendChild(selfOption);
